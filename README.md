...
> **上交启源** —— 专为医疗器械智能制造行业打造的高保真数据可视化大屏。
...
本项目是一个基于 **React 19** 的单页应用（SPA），旨在提供医疗器械生产、物流和质量控制全流程的实时概览。它具备中英双语界面、深度交互式图表以及实时数据可视化功能。

## ✨ 核心功能

*   **🌐 国际化支持**: 内置 React Context 状态管理，支持一键切换 **中文** 与 **英文** 界面。
*   **📊 深度交互图表**:
    *   **生产进度**: 带有滞后指标和状态追踪的条形图。
    *   **平台数据汇总**: 支持自定义和图例交互的环形图。
    *   **工作量统计**: 带有钻取功能（模态框详情）和时间维度筛选（周/月/周期）的动态仪表盘。
*   **🗺️ 混合地图系统**:
    *   **高德地图集成**: 支持接入 AMap (高德地图 SDK 2.0) 展示真实地理信息。
*   **🛡️ 健壮的数据层**:
    *   API 请求自动拦截：如果后端接口不可用，系统会自动使用内置的 **模拟数据** 进行展示。
